var SVGtoPDF = function(doc, svg, x, y, options) {
  ...
  const Properties = {
    ...
    'xml:space': {
      ...
      initial: 'default',
      css: 'white-space',
      values: { preserve: 'preserve', default: 'default', pre: 'preserve', 'pre-line': 'preserve', 'pre-wrap': 'preserve', nowrap: 'default' },
    },
  ...
  function docWriteGlyph(glyph) {
    // console.log('docWriteGlyph', glyph)
  ...
  function getTextPos(font, size, text) {
    ...
    for (let i = 0; i < hex.length; i++) {
      ...
      // console.log('[getTextPos], pos[i]', pos[i])
      data.push({
        glyph: hex[i],
        unicode: unicode,
  ...
  var SvgElemSvg = function(obj, inherits) {
    ...
    this.getTransformation = function() {
      return multiplyMatrix(this.get('transform'), [1, 0, 0, 1, x, y], parseAspectRatio(aspectRatio, width, height, viewBox[2], viewBox[3]), [
        1,
        0,
        0,
        1,
  ...
  var SVGElemImage = function(obj, inherits) {
    ...
    let link = imageCallback(this.attr('href') || this.attr('xlink:href') || ''),
      ...
      image
    try {
      // console.log('opening image '+link, typeof link === 'string')
        ...
        switch (childElem.name) {
          ...
          case '#cdata-section':
            ...
            if (fill || stroke || isClip) {
              ...
              // console.log('[drawTextInDocument] childElem._pos = ',childElem._pos)
              for (let j = 0, pos = childElem._pos; j < pos.length; j++) {
                if (!pos[j].hidden && isNotEqual(pos[j].width, 0)) {
                  ...
                  // console.log('[drawTextInDocument] pos[j].glyph = ',pos[j].glyph)
    ...
    ;(function(textParentElem) {
      ...
      function doAnchoring() {
        if (currentChunk.length) {
          ...
          let anchordx =
            { startltr: 0, middleltr: 0.5, endltr: 1, startrtl: 1, middlertl: 0.5, endrtl: 0 }[currentAnchor + currentDirection] * width || 0
        ...
        for (let i = 0; i < children.length; i++) {
          ...
          switch (childElem.name) {
            ...
            case '#cdata-section':
              ...
              let fontNameorLink = fontCallback(
              ...
              currentElem._font.font = doc._font
              ...
              // console.log('[SvgElemText.recursive] text ',rawText)
              for (let w = 0; w < words.length; w++) {
                ...
                for (let j = 0; j < pos.length; j++) {
                  ...
                  // console.log('[SvgElemText.recursive] glyph ',position.glyph, 'width', position.width)
          ...
          for (let j = 0; j < currentElem._pos.length; j++) {
            ...
            if (charMidX > pathLength || charMidX < 0) {
              ...
              currentElem._pos[j].x =
                pointOnPath[0] - 0.5 * currentElem._pos[j].width * Math.cos(pointOnPath[2]) - currentElem._pos[j].y * Math.sin(pointOnPath[2])
              currentElem._pos[j].y =
                pointOnPath[1] - 0.5 * currentElem._pos[j].width * Math.sin(pointOnPath[2]) + currentElem._pos[j].y * Math.cos(pointOnPath[2])
